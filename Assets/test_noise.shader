	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
                ...
                //float gShiftRandom = rShiftRandom;
                //float bShiftRandom = rShiftRandom;
				...
				fixed4 rShifted = tex2D(_MainTex, float2(IN.texcoord.x + 255, IN.texcoord.y + 255));
				fixed4 gShifted = tex2D(_MainTex, float2(IN.texcoord.x + 255, IN.texcoord.y + 255));
				fixed4 bShifted = tex2D(_MainTex, float2(IN.texcoord.x + 255, IN.texcoord.y + 255));
